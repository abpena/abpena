...
def count_anagrams(tree, word, prefix=""):
    if len(word) <= 1:
        word_2 = prefix + word
        if tree.search(word_2, tree.root):
            return 0
        else:
            return -1
    else:
...
def max_anagrams_helper(tree, node):
    if len(node.children) == 0:
        return

    for key in node.keys:
        word_dict[count_anagrams(tree, key, "")] = key

    for child in node.children:
        max_anagrams_helper(tree, child)


def max_anagrams(tree):
    max_anagrams_helper(tree, tree.root)
    max_num = 0
    for key in word_dict.keys():
        if key > max_num:
            max_num = key

    return word_dict[max_num], max_num
...
def main():
    ...
    while line:
        b_tree.insert(line[0])
        line = file.readline().split()

    file.close()
    print(count_anagrams(b_tree, "dichlorodiphenyltrichloroethane", ""))
    print(max_anagrams(b_tree))
    # print(b_tree.print_d(" "))
